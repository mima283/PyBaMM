...
def NMC_diffusivity_Clerici(sto,T):

    D0 = 8 * 10 ** (-15)  # D_ref v NMC_diffusivity_PeymanMPM
    Ω =  1.25e-5 # Chen2020 ["Positive electrode partial molar volume [m3.mol-1]"]
    E =  199e9 # Cheng https://doi.org/10.1016/j.jeurceramsoc.2017.03.048 ["Positive electrode Young's modulus [Pa]"]
    ν = 0.25 # Cheng https://doi.org/10.1016/j.jeurceramsoc.2017.03.048 ["Positive electrode Poisson's ratio"]
    Rg = 8.3145 # ["Ideal gas constant [J.K-1.mol-1]"]

    k = 2 * Ω **2 * E / (9 * Rg * T * (1 - ν))  # [m^3/mol]

    c_max = 35380.0 # Mohtat2020 ["Maximum concentration in positive electrode [mol.m-3]"]
    C = sto * c_max # [mol/m^3]

    return D0 * (1 + k * C)  # [m^2/s]

def graphite_diffusivity_Clerici(sto,T):
    
    D0 = 5.0 * 10 ** (-15)  # D_ref od PeymanMPM 
    # D0 = 3.9*10**(-14) # Ai2020
    Ω = 3.1e-6  # Ai2020 ["Negative electrode partial molar volume [m3.mol-1]"]
    E = 15e9 # Ai2020 ["Negative electrode Young's modulus [Pa]"]
    ν = 0.3 # Ai2020 ["Negative electrode Poisson's ratio"]
    Rg = 8.3145 # ["Ideal gas constant [J.K-1.mol-1]"]

    c_max = 28746.0 # ["Maximum concentration in negative electrode [mol.m-3]"]  
    C = sto * c_max # [mol/m^3]
...
def get_parameter_values():
    ...
    return {
        ...
        "Negative particle diffusivity [m2.s-1]": graphite_diffusivity_Clerici,
        ...
        "Positive particle diffusivity [m2.s-1]": NMC_diffusivity_Clerici,
